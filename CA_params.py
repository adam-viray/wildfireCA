...
class fireCA:
    ...
    def plot(self, path=False):
        ...
        ax[1].plot(self.FP[:j], ':r', label='false positive')
        ax[1].plot(self.FN[:j], ':b', label='false negative')
        ax[1].set_ylabel('# cells')
        ax[1].legend()
        ax2 = ax[1].twinx()
        ax2.set_ylim(0,1)
        ax2.plot(self.kappa[:j], '-.g', alpha=0.5, label='agreement')
        ...
        if len(spreading) > 10:
            with Pool(8) as p1:
                try:
                    ...
                    print('inner pool closing')
                    p1.shutdown(wait=False, cancel_futures=True)
        else:
    ...
    def run(self):
        ...
        while self.i < (self.t-1)*self.consts['s_base']:
            ...
            if len(self.burning) > 1000:
                ...
                self.kappa_final = self.kappa[self.j]
                if self.plotting:
                    self.plot(self.consts['gif_dir']+f"{self.day:02}_{self.hour:02}:{self.minute:02}:{self.second:02}.png")
                plt.close('all')
                break
            if (self.j > 12 and (np.sqrt(np.sum(np.square(self.kappa))) < 0.5 or self.kappa[self.j] < 0.01)):
                print("Model agreement too low")
                self.kappa_final = self.kappa[self.j]
                if self.plotting:
                    self.plot(self.consts['gif_dir']+f"{self.day:02}_{self.hour:02}:{self.minute:02}:{self.second:02}.png")
                plt.close('all')
            ...
            if self.i > self.consts['s_base']*(24*self.day+self.hour):
                ...
                ToTal = len(CA_pixels.union(VIIRS_pixels))
                TP = len(CA_pixels.intersection(VIIRS_pixels))
                self.kappa[self.j] = TP/ToTal
                self.FP[self.j] = len(CA_pixels - VIIRS_pixels)
                self.FN[self.j] = len(VIIRS_pixels - CA_pixels)
                if self.hour != self.j%24:
                    self.hour = self.j%24
                    self.wind_spd = self.wspd[self.j]
                    self.wind_dir = self.wdir[self.j]
            ...
            if ((self.n_burnt - self.n_prev_burnt > 0.05*self.n_prev_burnt) or (self.i - self.i_prev > 6*self.consts['s_base'])):
...
def doCA(p):
    d = '/home/adam.viray/Documents/CA/data/test_fires/MT4644711368920170713/120m/'
    incname = re.findall('[A-Za-z]{2}[0-9]{10,}',d)[0]
    fname = f'{incname}_{p[0]:.2f}_{p[1]:.2f}_{p[2]:.2f}'
    CA = fireCA(d)
    CA.consts['spread_threshold'] = p[0]
    CA.consts['initial_burn'] = p[1]
    CA.consts['burn_speed'] = p[2]
    ...
    out_dir = '/home/adam.viray/Documents/CA/output/params/'
    
    np.savetxt(out_dir+f'{fname}_stats_{CA.kappa_final:.3f}.csv', np.vstack((CA.CA_burnt_number, CA.FP, CA.FN, CA.kappa)).T, delimiter=',', header='model, FP, FN, kappa')
    CA.plot(out_dir+f'{fname}_plot_{CA.kappa_final:.3f}.png')
    
    return CA.kappa_final
    
def main():
    spread_threshold = [0.8,0.82]
    initial_burn = [0.41,0.43]
    burn_speed = [0.17,0.19]
    params = np.array(np.meshgrid(spread_threshold, initial_burn,burn_speed)).T.reshape(-1,3)
    
    with Pool(max_workers=8) as p0:
        try:
            k = list(p0.map(doCA, params))
        except KeyboardInterrupt:
            print('outer pool closing')
            p0.shutdown(wait=False, cancel_futures=True)
    
    plt.bar(np.arange(len(k)),k,label=[f'{p[0]} {p[1]} {p[2]}' for p in params])
    plt.savefig('/home/adam.viray/Documents/CA/output/params/MT4644711368920170713.png')
